<template>
	<view class="content">
		...
		<view class="setDefault">
			<text>默认地址</text>
			<switch @change="switchCh" :checked="this.query.default==true"></switch>
		</view>
		
		<view class="saveAdress" @click="editAddress">
			修改地址
		</view>
...
<script>
	...
	export default {
			...
			methods:{
				getData(id){
					this.$request('/api/member/getAddressInfo',{id:id},'post').then(res=>{
						console.log(res);
				...
				editAddress(){
					this.$request('/api/member/addressEdit',this.query,'post').then(res=>{
						console.log(res);
						if(res.data.code==1){
							uni.showToast({
								title:res.data.msg,
